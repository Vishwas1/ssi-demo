"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var db_service_1 = require("./db.service");
var lds_sdk_1 = require("lds-sdk");
var Application = /** @class */ (function () {
    function Application(_a) {
        var _b = _a.appId, appId = _b === void 0 ? " " : _b, _c = _a.appSecret, appSecret = _c === void 0 ? " " : _c, _d = _a.name, name = _d === void 0 ? " " : _d, _e = _a.userId, userId = _e === void 0 ? " " : _e, _f = _a.isActive, isActive = _f === void 0 ? " " : _f;
        this.appId = appId; // new uuid
        this.id = this.appId;
        this.appSecret = appSecret;
        this.isActive = isActive;
        this.name = name;
        this.userId = userId;
        this.dbSerice = new db_service_1.DBService();
        this.prefix = 'app_';
    }
    Application.prototype.toString = function (user) {
        return JSON.stringify(user);
    };
    Application.prototype.getId = function () {
        var uuid = this.prefix + lds_sdk_1.getChallange();
        return uuid.substring(0, 20);
    };
    Application.prototype.create = function () {
        return __awaiter(this, void 0, void 0, function () {
            var newUser;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.appId = this.getId();
                        this.id = this.appId;
                        this.appSecret = this.getId();
                        this.isActive = "true";
                        return [4 /*yield*/, this.dbSerice.add(db_service_1.SchemaType.Application, this)];
                    case 1:
                        newUser = _a.sent();
                        return [2 /*return*/, this.toString(newUser)];
                }
            });
        });
    };
    Application.prototype.fetch = function () {
        return __awaiter(this, void 0, void 0, function () {
            var props, queryParams, users;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        props = Object.getOwnPropertyNames(this);
                        queryParams = {};
                        props.forEach(function (e) {
                            if (e == 'dbSerice')
                                return;
                            if (e == 'prefix')
                                return;
                            if (_this[e] != " ")
                                queryParams[e] = _this[e];
                        });
                        return [4 /*yield*/, this.dbSerice.getAll(db_service_1.SchemaType.Application, queryParams)];
                    case 1:
                        users = _a.sent();
                        return [2 /*return*/, users];
                }
            });
        });
    };
    return Application;
}());
exports.Application = Application;
